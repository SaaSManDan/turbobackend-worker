...
`;
  ...
  // Commit to git
  await executeCommandInContainer(containerId, 'git add fly.toml Dockerfile');
  await executeCommandInContainer(containerId, 'git commit -m "Add Fly.io configuration and Dockerfile"');
  await executeCommandInContainer(containerId, 'git push origin main');

  console.log('[Fly.io] fly.toml and Dockerfile created');
}
